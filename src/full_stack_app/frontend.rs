use aws_cdk_lib::{
    aws_amplify::{self, CfnAppProps, CfnAppTrait, CfnBranchProps},
    aws_iam::{self, IRoleTrait, ManagedPolicyTrait},
};
...
impl Frontend {
    pub fn new(
        ...
        let amplify_role = aws_iam::Role::new(
            stack,
            "AmplifyDeployRole".to_string(),
            aws_iam::RoleProps {
                assumed_by: Box::new(aws_iam::ServicePrincipal::new(
                    "amplify.amazonaws.com".to_string(),
                    None,
                )),
                managed_policies: Some(vec![aws_iam::ManagedPolicy::from_aws_managed_policy_name(
                    "service-role/AmplifyBackendDeployFullAccess".to_string(),
                )]),
                ..Default::default()
            },
        );

        let amplify = aws_amplify::CfnApp::new(
            stack,
            "MyDataProjectAmplifyInstance".to_string(),
            aws_amplify::CfnAppProps {
        ...
        let _branch = aws_amplify::CfnBranch::new(
            stack,
            "MyDataProjectAmplifyBranch".to_string(),
            aws_amplify::CfnBranchProps {
